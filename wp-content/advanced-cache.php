...
$possible_plugin_locations = array(
	defined('WP_PLUGIN_DIR') ? WP_PLUGIN_DIR.'/wp-optimize_/cache' : false,
	defined('WP_CONTENT_DIR') ? WP_CONTENT_DIR.'/plugins/wp-optimize_/cache' : false,
	dirname(__FILE__).'/plugins/wp-optimize_/cache',
	'/var/www/html/wp-content/plugins/wp-optimize_/cache',
);
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', '/var/www/html/wp-content/plugins/wp-optimize_/cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-aeon2.blvckpixel.com.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-aeon2.blvckpixel.com.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-aeon2.blvckpixel.com.php');
}
