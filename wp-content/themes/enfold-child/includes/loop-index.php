...
if(is_user_logged_in()){
	$sd_message="";
	if($_SESSION['sd_messagemessage']!=""){
		$sd_message=$_SESSION['sd_message'];
	...
	$sharinguser = $user->display_name;
	$user_roles = $user->roles;
	if ( in_array( 'company_user', $user_roles, true ) ) {
		//user is company user show share document option
	

?>

<!-- Code for shared document plugin  -->

<div id="share-co" class="share">

	<button id="sh-list">Share Document With My Team <i class="fa fa-share"></i></button>
	<script>
	function CheckAll(){
		var elements = document.getElementsByName("co_list[]");
	...
	jQuery(document).ready(function(){
		jQuery("#sh-list").click(function(){
			jQuery(".co-user-list").slideToggle('fast').css('display','block');
		});
	});
</script>
</div>
...
<div class="co-user-list" style="display: none;">
	<div class="row">
		<div class="col-sm-6">
			<h3>Share With:</h3>
		</div>
		<div class="col-sm-6 text-right">
			<button >Select All <input name="ALL" type="checkbox" id="check-all2" value="checkbox" onclick="CheckAll()" /></button>
		</div>
	</div>
	<?php 
	$args = array(
	...
	foreach ( $users as $user ) {
		...
		if($fetchuserid!=$userid){
			...
			if($fetchusercompany==$usercompany){
				...
				echo '<table><td><input type="checkbox" name="co_list[]" id="co_list" value='.$fetchuserid.'> ' . esc_html( $v1 ) . ' '.'[' . esc_html( $v2 ) . ']</td></table>';
			}
		}	
	}
	echo '</ul>';
	echo '<input type="submit" name="sharepost" id="sharepost" value="Share Document">';
	echo '</form>';

	//SUBMIT 
	if(isset($_POST['sharepost']) && !empty($_POST['co_list'])){
		//insert into db
		$sharedoc_table= $wpdb->prefix . "sharedocs";
		
		$sharedwith=$_POST['co_list'];
		foreach($sharedwith AS $swith){
				$usermeta = get_userdata( $swith );
				$useremail= $usermeta->user_email;
				$current_date_time=date("Y-m-d_H-i-s");

			//get email of user
			if($wpdb->insert($sharedoc_table,array(
				'sharedby'=>$userid,
				'sharedto' =>$swith,
				'postid' =>$postid,
				'sharedon' =>$current_date_time),
				array('%s','%s','%s','%s'))== false) $_SESSION['sd_message']="Something went wrong.Please try again"; else 
				$_SESSION['sd_message']="Document Shared Successfully";

				//MAIL FUNCTION TO SHARE POST
				$to = $useremail;
				$subject = 'New Document Shared With You';
				$headers = array('Content-Type: text/html; charset=UTF-8');
				$posttitle = get_the_title($postid);
				$postlink = get_permalink($postid);
				$html = "The Following Document was shared to you by $sharinguser <br> Document Title: $posttitle <br> Document Link: $postlink";
				$body = $html;
				wp_mail( $to, $subject,  $body, $headers);
		}
	}else{
		$_SESSION['sd_message']="Please select team member to share document";	
	}
...
<!-- End code of shared document plugin -->
